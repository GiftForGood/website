...
/**
 *
 * @param {string} postType is type of post of the new chat
 */
const NewChatTips = ({ postType }) => {
  if (postType === wishes) {
    return (
      <MessageContainer>
        <MessageTitle>
    ...
    // decide on the tips to show when npo that initiates a chat for a donation
    return <div>TBC.</div>;
  }
};

export default NewChatTips;
